...
La terminal (también conocida como consola, línea de comandos o shell)
es una herramienta que permite interactuar directamente con el sistema
operativo mediante texto. En lugar de usar una interfaz gráfica (como
ventanas y botones), en la terminal escribes comandos para realizar
tareas como:
...
Windows: Tiene el símbolo del sistema (CMD) y PowerShell. También se
puede usar Windows Terminal, que permite múltiples shells.
...
Un programa que se puede ejecutar desde la terminal. Este puede recibir
...
```
- comando: Es la acción que quieres ejecutar (por ejemplo, ls, cd, mkdir, grep, etc.).
